                            ...
                            parallelize = FALSE, n_cores = 2, batch_size = 100) 
{
  ...
  for (i in seq_len(p)) {
    aniso_list_t = aniso_precomp_list[[i]]
    for (t_idx in seq_len(nt)) {
      for (s_idx in seq_len(ns)) {
        AA = aniso_list_t[[t_idx]][[s_idx]]      # (2 x 2) matrix
        aniso_5D[i, t_idx, s_idx, , ] = AA
        det_array_all[i, t_idx, s_idx] = det(AA)
      }
    }
  }
  ...
  for(i in seq_len(p)) {
    nu_3D[i, , ] = matern_precomp_list[[i]]$nu
    r_3D[i,  , ] = matern_precomp_list[[i]]$r
  }
  ...
  SigmaFun_4D = array(0, dim = c(nt, ns, p, p))
  for(t_idx in seq_len(nt)) {
    for(s_idx in seq_len(ns)) {
  ...
  single_iteration = function(iter) {
    ...
    pdfXi_Xil = pdfXi(Xil)
    x = sqrt(2) * Vl
    pdfi      = GaussianSpectralDensity(x, Sigma =  diag(d)) 
  ...
  if(!parallelize) {
    ...
    for (batch in seq_len(num_batches)) {
      ...
      Z_list = mclapply(batch_indices, single_iteration, mc.cores = 1)
...
############################# For temporal simulations #################

TemporalSim = function(nt,r_t,rho,a,nu,A){
  # nt = number of time steps
  # r  = a p-vector of scale parameters 
  # rho = a p x p matrix of correlation 
  # a = scale parameters for spatial Matern (default is 1)
  # nu = regul. parameters for spatial Matern (default is 1)
  # A = parameters in the pseudo-variogram (default is 0)
  
  # For C^(1)
  p = length(r_t)
  W = array(0,dim=c(nt,p))
  rt_ij = sqrt(0.5*outer(r_t^2,r_t^2,'+'))
  w = sqrt(outer(r_t,r_t,'*'))/rt_ij
  beta = rho*w
  ...
  beta = beta*a_factor*nu_factor*A_ij

  # Indep simulation
  for (k in seq_len(p)){
    Cov = exp(-r_t[k]*abs(outer(seq_len(nt),seq_len(nt),'-')))
    W[,k] = t(chol(Cov))%*%rnorm(nt)
  }
  
  # Cholesky multiplication
  Zt = W %*% t(chol(beta))
}
...
Gneiting_Matern_correlation = function(i, j, s1, s2, t1, t2, params, sigma_ij_x1x2, spatial_coordinates) {
  ...
  R_ij =  r_jj_x2 * r_ii_x1 / gamma0_t1_t2
...
SimulateGauIncrements = function(params) {
  ...
  #          (A_i), variogram parameters (a, alpha, b), etc.
  #
  # Returns:
  ...
  A = params$A          # List of parameters A
  ...
  Sigma = matrix(0, nrow = p*n, ncol = p*n)
  ...
  gamma0_tm_tn = gamma0(delta_t, a, alpha)
  
  #R_ij_0 = outer(A^2,A^2,'+')/2
  for (i in 1:p) {
    for (j in 1:p) {
      ...
      # Compute R_ij(tm, tn)
      R_ij_tm_tn =  A[i]*A[j] / (1+gamma0_tm_tn)
      R_1i_tm    =  A[1]*A[i] / (1+gamma0_t)
      R_1j_tn    =  A[1]*A[j] / (1+gamma0_t)
      
      Cov =  -gamma0_tm_tn + outer(gamma0_t,gamma0_t,'+') 
      Cov = Cov + R_ij_tm_tn - outer(R_1i_tm,R_1j_tn,'+') + A[1]^2

      Sigma[rows, cols] = Cov 
    }
  }
  Sigma = Sigma + diag(1e-6, nrow = ncol(Sigma))
  chS = chol(Sigma)
  return(t(chS))
}
