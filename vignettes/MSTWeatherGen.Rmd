...
The data considered here is the meteorological dataset SAFRAN, developed by Météo-France (starting in 2011/01/01 and ending in 2021/12/31). We only consider a small region in the south of France for the period 2018-2021 (to reduce computation time). We consider 3 variables: precipitation, wind, and maximum temperature.
...
index_dates <- real_dates %in% dates
...
swg <- MSTWeatherGen_Estim(
  data = data, seasons = seasons, dates = dates, names = names, by_season = T, scale = T,
...
```{r echo=TRUE}
sim <- MSTWeatherGen_Sim(
  dates_sim = dates, dates_original = dates,
...
ggplot2::ggplot(df_long, ggplot2::aes(x = Time, y = Value, color = Type)) +
  ...
  ggplot2::labs(
    title = "Observed vs simulated maximum Temperature",
    x = "Time",
    y = "Value",
    color = "Type"
  ) +
...
plot_observed_vs_simulated_density(
  sim = sim, observed = data, dates = dates,
...
plot_dry_wet_spells_maps(sim = sim[id,,,drop=FALSE], observed = data[id,,,drop=FALSE], coordinates = coordinates, dates = dates[id])
```
