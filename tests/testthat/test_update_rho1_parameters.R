...
par_all_TEST_updated <- update_rho1_parameters(par_all_TEST, names, extract_rho1(create_df_param(par_all_TEST, names), names))
...
# 0.
test_that("update_rho1_parameters runs without error with matrix input", {
  ax_matrix <- matrix(
    c(
      1, 0.5, 0.3,
      0.5, 1, 0.4,
...
test_that("nearPD is applied to non-positive definite matrix", {
  ...
  result <- update_rho1_parameters(par_all_TEST, names, ax_matrix_non_PD)
...
test_that("all zeros matrix (extreme case)", {
  ...
  expect_error(result <- update_rho1_parameters(par_all_TEST, names, ax_zeros))
})
...
test_that("function is deterministic with same inputs", {
  ...
  result1 <- update_rho1_parameters(par_all_TEST, names, ax_matrix)
  result2 <- update_rho1_parameters(par_all_TEST, names, ax_matrix)
...
#1.
test_that("update_rho1_parameters univarié : tourne sans erreur", {
  expect_no_error(
    result1 <- update_rho1_parameters(par_all_univ, names_univ, ax_univ)
  )
})
