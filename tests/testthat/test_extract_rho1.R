...
par_all_TEST_updated <- update_beta1_parameters(par_all_TEST, names, extract_rho1(create_df_param(par_all_TEST, names), names))
parm <- create_df_param(par_all_TEST_updated, names)
...
# 0.
test_that("extract_rho1 runs without error", {
  expect_silent({
    result <- extract_rho1(parm, names)
  })
...
# 1.
test_that("extract_rho1 returns correct matrix dimensions", {
  result <- extract_rho1(parm, names)
...
# 2.
test_that("extract_rho1 returns matrix with correct names", {
  result <- extract_rho1(parm, names)
...
test_that("function is deterministic", {
  result1 <- extract_rho1(parm, names)
  result2 <- extract_rho1(parm, names)

  expect_identical(result1, result2)
})

#5.
test_that("extract_rho1 univarié (length(names)==1) retourne une matrice", {
